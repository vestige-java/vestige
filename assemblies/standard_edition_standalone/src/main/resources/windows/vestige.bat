...
set VESTIGE_OPTS=%VESTIGE_OPTS% -Dvestige.mavenRepository="%DIRNAME%\repository" -Djava.util.logging.manager=com.googlecode.vestige.core.logger.JULLogManager -Dlogback.logsDirectory="%VESTIGE_BASE%\logs" -Dlogback.configurationFile="%LOGBACK_CONFIGURATION_FILE%"

"%JAVA%" %VESTIGE_OPTS% -jar "%LIBDIR%\vestige.core-${vestige.core.version}.jar" "%LIBDIR%\vestige.assemblies.standard_edition_bootstrap-${project.version}-jar-with-dependencies.jar" com.googlecode.vestige.jvm_enhancer.JVMEnhancer com.googlecode.vestige.resolver.maven.VestigeMavenResolver "%MAVEN_LAUNCHER_FILE%" "%MAVEN_SETTINGS_FILE%" "%MAVEN_RESOLVER_CACHE_FILE%" "%VESTIGE_BASE%" "%VESTIGE_DATA%" %VESTIGE_LISTENER_PORT% || exit /B 2
