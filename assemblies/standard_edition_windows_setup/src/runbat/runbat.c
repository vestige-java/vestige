				   ...
				   LPSTR lpCmdLine, int nCmdShow)
{
	...
	char * batfile = malloc(len + 1);
	strcpy(batfile, __argv[0]);
	char * charptr = batfile + len - 3;
	strcpy(charptr, "bat");
	charptr--;
	while (*charptr != '\\') {
		charptr--;
	...
	charptr++;
	int appLen = (batfile - charptr) + len - 4;

	char * appName = malloc(appLen + 1);
	strncpy(appName, charptr, appLen);
